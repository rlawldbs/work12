...
def year_archive(request, year):
    if year is not None:
        return HttpResponse('{}년도 자료 입니다.' .format(year)) #url로부터 year라는 변수 들어옴
    else:
        return HttpResponse('해당년도 자료는 없습니다.')

def my_sum(request, x, y):
    result =  x + y #두개의 값을 더하고 아랫줄에선 문자열로 바꿔줌
    output =  '{} = {} + {}'.format(result, x, y) #{}(결과)는 {}(x)더하기 {}(y)이다. 문자열로 바꿔줌
    return HttpResponse(output) #render는 이미 내부적으로 HttpRespons를 쓰기 때문에 필요 없음
